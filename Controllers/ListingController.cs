    ...
    public class ListingController : AuctionWorxController
    {
        ...
        public ActionResult Browse(string breadcrumbs, int? page, int? SortFilterOptions, string ViewStyle, string StatusFilter)
        {
            ...
            try
            {
                ...
                {
                    results = ListingClient.SearchListingsWithFillLevelAndContext(User.Identity.Name, currentQuery, page.Value, SiteClient.PageSize, fillLevel, this.FBOUserName());
                }
                else
                {
                    results = ListingClient.SearchListingsWithFillLevel(User.Identity.Name, currentQuery, page.Value, SiteClient.PageSize, fillLevel);
                }
                ...
                try
                {
                    ...
                    if (!currentListing.Properties.Any(p => p.Field.Name.Equals("ReservePrice")))
                    {
                        ...
                        string.IsNullOrEmpty(
                            currentListing.Properties.Where(p => p.Field.Name.Equals("ReservePrice")).Single().Value))
                    {
                        ViewData["ReserveStatus"] = "NoReserve";
                    }
                    else if (currentListing.CurrentPrice.Value >=
                             decimal.Parse(
                                 currentListing.Properties.Where(p => p.Field.Name.Equals("ReservePrice"))
                                     .Single()
                                     .Value))
                    {
                    ...
                    else
                    {
                        ...
                        if (User.Identity.IsAuthenticated && (currentListing.Status == ListingStatuses.Successful || (currentListing.Type.Name == ListingTypes.FixedPrice && currentListing.AcceptedListingActionCount() > 0)))
                        {

                            //find any uninvoiced lineitems to this buyer and generate an invoice, if needed
                            if (currentListing.OwnerAllowsInstantCheckout() && !currentListing.OwnerUserName.Equals(this.FBOUserName()))
                            {
                                //get all uninvoiced sales for this listing, to this buyer
                                var unInvoicedLineItems = AccountingClient.GetListingLineItemsBySeller(currentListing.OwnerUserName, currentListing.OwnerUserName,
                                    "NeedInvoice", currentListing.ID.ToString(), "ListingID", 0, 0, "DateStamp", true).List
                                    .Where(i => i.Payer.UserName.Equals(this.FBOUserName())).ToList();
                                if (unInvoicedLineItems.Count > 0)
                                {
                                    //get all unpaid invoices from this seller to this buyer to see if any of them are appropriate to add this line item to
                                    var unpaidInvoicesTothisBuyer = AccountingClient.GetInvoicesBySeller(currentListing.OwnerUserName, currentListing.OwnerUserName,
                                        "Unpaid", this.FBOUserName(), "User", null, 0, 0, Fields.CreatedDTTM, true).List
                                        .Where(inv => inv.Status != InvoiceStatuses.Pending && inv.Payer.UserName.Equals(this.FBOUserName()));
                                    foreach (var newLineItem in unInvoicedLineItems)
                                    {
                                        ...
                                        foreach (var potentialInvoice in unpaidInvoicesTothisBuyer)
                                        {
                                            ...
                                            if (similarLineItems.Any(li => li.ID == newLineItem.ID))
                                            {
                                                ...
                                                AccountingClient.AddLineItemToInvoice(currentListing.OwnerUserName, invoice.ID, newLineItem.ID);
                                                break;
                                            }
                                        }
                                        if (invoice == null)
                                        {
                                            ...
                                            invoice = AccountingClient.CreateInvoiceFromLineItem(currentListing.OwnerUserName, newLineItem.ID);
                                        }
                                    }
                                }
                            }
        ...
        public ActionResult CreateListingPage1(int? SimilarListingID, string ReturnUrl)
        {
            ...
            if (SiteClient.BoolSetting(SiteProperties.RequireCreditCardForSellers) && !User.IsInRole("Admin") && !this.HasUnexpiredCardOnFile(null, this.FBOUserName()))
            {
        ...
        public ActionResult ConfirmActionModal(int ListingID)
        {
            ...
            try
            {
                ...
                if (SiteClient.BoolSetting(SiteProperties.RequireCreditCardForBuyers) && !User.IsInRole("Admin") && !this.HasUnexpiredCardOnFile(listing.OwnerUserName, this.FBOUserName()))
                {
        ...
        public ActionResult Action(int listingID)
        {
            ...
            try
            {
                ...
                else if (accepted && newPurchaseLineitem != null)
                {
                    ...
                    if (listing.OwnerAllowsInstantCheckout())
                    {
        ...
        {
            var listing = ListingClient.GetListingByIDWithFillLevel(User.Identity.Name, ListingID, ListingFillLevels.None);

            ViewData["CCRequired"] = false;
            if (User.Identity.IsAuthenticated && SiteClient.BoolSetting(SiteProperties.RequireCreditCardForBuyers) && !User.IsInRole("Admin") && !this.HasUnexpiredCardOnFile(listing.OwnerUserName, this.FBOUserName()))
            {
                PrepareNeutralMessage(Messages.CreditCardRequiredForBuyers);
                //return RedirectToAction(Strings.MVC.AddCardAction, Strings.MVC.AccountController, new { returnUrl = Request.Url.PathAndQuery });
                ViewData["CCRequired"] = true;
        ...
        public ActionResult ConfirmOfferModal(int ListingID)
        {
            ...
            else
            {
                ...
                try
                {
                    ...
                    if (SiteClient.BoolSetting(SiteProperties.RequireCreditCardForBuyers) && !User.IsInRole("Admin") && !this.HasUnexpiredCardOnFile(newOffer.ListingOwnerUsername, this.FBOUserName()))
                    {
                        PrepareNeutralMessage(Messages.CreditCardRequiredForBuyers);
                        //return RedirectToAction(Strings.MVC.AddCardAction, Strings.MVC.AccountController, new { returnUrl = Request.Url.PathAndQuery });
                        ViewData["CCRequired"] = true;
        ...
        public ActionResult History(int id, string currency, string sort, int? page, bool? descending)
        {
            ...
            string fillLevel = ListingFillLevels.Actions + "," + ListingFillLevels.CurrentAction;
