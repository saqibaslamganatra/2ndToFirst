                ...
                try
                {
                    ...
                    if (!currentLot.Listing.Properties.Any(p => p.Field.Name.Equals("ReservePrice")))
                    {
                        ...
                        string.IsNullOrEmpty(
                            currentLot.Listing.Properties.Where(p => p.Field.Name.Equals("ReservePrice")).Single().Value))
                    {
                        ViewData["ReserveStatus"] = "NoReserve";
                    }
                    else if (currentLot.Listing.CurrentPrice.Value >=
                             decimal.Parse(
                                 currentLot.Listing.Properties.Where(p => p.Field.Name.Equals("ReservePrice"))
                                     .Single()
                                     .Value))
                    {
                    ...
                    else
                    {
                        ...
                        if (User.Identity.IsAuthenticated && (currentLot.Listing.Status == ListingStatuses.Successful || (currentLot.Listing.Type.Name == ListingTypes.FixedPrice && currentLot.Listing.AcceptedListingActionCount() > 0)))
                        {

                            //find any uninvoiced lineitems to this buyer and generate an invoice, if needed
                            if (currentLot.Listing.OwnerAllowsInstantCheckout() && !currentLot.Listing.OwnerUserName.Equals(this.FBOUserName()))
                            {
                                //get all uninvoiced sales for this listing, to this buyer
                                var unInvoicedLineItems = AccountingClient.GetListingLineItemsBySeller(currentLot.Listing.OwnerUserName, currentLot.Listing.OwnerUserName,
                                    "NeedInvoice", currentLot.Listing.ID.ToString(), "ListingID", 0, 0, "DateStamp", true).List
                                ...
                                if (unInvoicedLineItems.Count > 0)
                                {
                                    ...
                                    var unpaidInvoicesTothisBuyer = AccountingClient.GetInvoicesBySeller(currentLot.Listing.OwnerUserName, currentLot.Listing.OwnerUserName,
                                        "Unpaid", this.FBOUserName(), "User", currentLot.Event.ID, 0, 0, Fields.CreatedDTTM, true).List
                                    ...
                                    foreach (var newLineItem in unInvoicedLineItems)
                                    {
                                        ...
                                        foreach (var potentialInvoice in unpaidInvoicesTothisBuyer)
                                        {
                                            ...
                                            if (similarLineItems.Any(li => li.ID == newLineItem.ID))
                                            {
                                                ...
                                                AccountingClient.AddLineItemToInvoice(currentLot.Listing.OwnerUserName, invoice.ID, newLineItem.ID);
                                                break;
                                            }
                                        }
                                        if (invoice == null)
                                        {
                                            ...
                                            invoice = AccountingClient.CreateInvoiceFromLineItem(currentLot.Listing.OwnerUserName, newLineItem.ID);
                                        }
                                    }
                                }
                            }
                ...
                if (currentEvent != null)
                {
                    ...
                    if (viewingAllowed)
                    {
                        ...
                        try
                        {
                            ...
                            {
                                results = ListingClient.SearchListingsWithFillLevelAndContext(User.Identity.Name, currentQuery, page.Value, effectivePageSize, fillLevel, this.FBOUserName());
                            }
                            else
                            {
                                results = ListingClient.SearchListingsWithFillLevel(User.Identity.Name, currentQuery, page.Value, effectivePageSize, fillLevel);
                            }
        ...
        public ActionResult MoreLots_Inline(int EventID, string ViewStyle, int gtLotOrder)
        {
            ...
            try
            {
                Page<Listing> results = EventClient.SearchLotsByEvent(SystemActors.SystemUserName, EventID, 
                    ...
                    0, SiteClient.PageSize, "LotOrder", false, fillLevel);
                return View(results);
            }
            catch (Exception)
            {
