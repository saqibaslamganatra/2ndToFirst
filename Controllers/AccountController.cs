    ...
    public class AccountController : AuctionWorxController
    {
                    ...
                    else
                    {
                        ViewData[Strings.Fields.ShippingOption] =
                            string.Format(Strings.Formats.ShippingMethodName, invoice.ShippingOption.Method.Name
                                , SiteClient.FormatCurrency(
                            /*invoice.ShippingOption.Amount*/invoice.ShippingAmount,
        ...
        public ActionResult PayAllInvoices(int? eventId, string returnUrl)
        {
            ...
            if (eventId.HasValue)
            {
                if (failedCount > 0)
                {
                    return RedirectToAction(Strings.MVC.InvoiceEventSalesAction, new { EventID = eventId.Value, ViewFilterOption = "Unpaid" });
                }
                else
                {
            ...
            else
            {
                if (failedCount > 0)
                {
                    return RedirectToAction(Strings.MVC.InvoiceSalesAction, new { ViewFilterOption = "Unpaid" });
                }
                else
                {
                    return RedirectToAction(Strings.MVC.InvoiceSalesAction);
                }
            }
        }
        ...
        public ActionResult PrintInvoice(int id)
        {
            ...
            if (invoice.Type != Strings.InvoiceTypes.Fee)
            {
                if (invoice.Type == Strings.InvoiceTypes.Shipping)
                {
                    if (invoice.Status != Strings.InvoiceStatuses.Paid)
                    {
        ...
        public ActionResult Checkout(int lineitemid, string returnUrl)
        {
            try
            {
                var newLineItem = AccountingClient.GetLineItemByID(User.Identity.Name, lineitemid);
                ...
                if (newLineItem.Listing.OwnerAllowsInstantCheckout())
                {
                ...
                try
                {
                    foreach (int listingId in allIds.Split(',').Select(s => int.Parse(s)))
                    {
                        ...
                        try
                        {
                            ...
                            FillInputFromListing(input, listing);
        ...
        private void FillInputFromListing(UserInput input, Listing listing)
        {
        ...
        public async Task<ActionResult> Register(string UserName)
        {
            ...
            if (SiteClient.BoolSetting(SiteProperties.EnableRecaptchaForRegistration))
            {
        ...
        public ActionResult ReadMessage(int id, string returnUrl)
        {
            ...
            UserMessage message = UserClient.ReadMessage(User.Identity.Name, id);
        ...
        #region Bulk Lot Copying/Moving/Deleting
        ...
        #endregion Bulk Lot Copying/Moving/Deleting
